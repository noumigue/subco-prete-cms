...
async function computeAlertes(strapi, appel) {
  ...
  for (const m of mesures) out.push({ icon: '🛠️', titre: 'Mesure corrective ouverte', detail: `${m.subvention?.candidature?.numeroDossier || 'Subvention'} — ${m.description}`, lien: m.subvention?.documentId ? `/gestion/subventions/${m.subvention.documentId}` : '/gestion/subventions' });
  for (const j of jalons) out.push({ icon: '📅', titre: 'Jalon dépassé', detail: `${j.subvention?.candidature?.numeroDossier || 'Subvention'} — « ${j.etape?.libelle || 'jalon'} » prévu le ${j.datePrevue}, non réalisé`, lien: j.subvention?.documentId ? `/gestion/subventions/${j.subvention.documentId}` : '/gestion/subventions' });
  return out;
}
